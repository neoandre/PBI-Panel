...
export class Visual implements IVisual {
  ...
  public update(options: VisualUpdateOptions): void {
    ...
    const nameSize = this.num(objects, 'nameText', 'fontSize', 14);
    const nameColor = this.col(objects, 'nameText', 'color', '#0F172A');
    ...
    if (table && table.rows && table.rows.length > 0) {
      ...
      if (idxMeasure >= 0) { value = row[idxMeasure]; measureName = cols[idxMeasure].displayName || measureName; }
  ...
  private formatValue(v: any): string {
    ...
    const num = Number(v); if (!isNaN(num)) return this.compact(num);
    return String(v);
  }
  ...
  private colorFromCondition(cond: any, fallback: string): string {
    ...
    const n = Number(s); if (!isNaN(n)) { if (n>0) return '#28FF18'; if (n===0) return '#FFEA04'; return '#FF2C2C'; }
    return fallback;
  }
}
